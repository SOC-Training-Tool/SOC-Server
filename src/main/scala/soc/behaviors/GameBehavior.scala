...
//  }
  ...
  def gameBehavior2[GAME <: Inventory[GAME], BOARD <: BoardConfiguration](config: GameConfiguration[GAME, BOARD], gc: GameContext[GAME, BOARD], moveResultProvider: MoveResultProvider[GAME]) = Behaviors.setup[Behavior2Messages] { context =>

    implicit val timeout: Timeout = 60.seconds
    implicit val scheduler = context.system.scheduler
    implicit val ec: ExecutionContextExecutor = context.executionContext

    val resultProvider = context.spawn[MoveResultProviderMessage[GAME]](MoveResultProvider.moveResultProvider(moveResultProvider), s"${config.gameId.key}_result_provider")

    def getNextAction: Unit = {
      gc.getNextAction.foreach { action =>
        context.pipeToSelf(action) {
          case Success(r@ReceiveMoveFromClient(id, move)) if gc.canDoMove(move) =>
            r.moveResponse.success(ProtoMoveResponse("SUCCESS"))
            ToMoveResult(move, gc.getGameState, id)
    ...
    Behaviors.receiveMessage[Behavior2Messages] {
      ...
      case ToMoveResult(move, state: GameState[GAME], id) =>
        context.ask[GetMoveResultProviderMessage[GAME], ResultResponse](resultProvider)(ref => GetMoveResultProviderMessage[GAME](state, id, move, ref)) {
          case Success(ResultResponse(_, result: DiscardResourcesResult)) =>
            context.log.debug(s"$id: $result")
            gc.updateGameState(id, result)
            if (gc.getGameState.phase != GamePhase.Discard) MoveRequest2
            else Empty
      ...
      case MoveRequest2 if gc.getGameState.isOver =>
        gc.onComplete
        Behaviors.stopped
  ...
  case object Start extends Behavior2Messages
