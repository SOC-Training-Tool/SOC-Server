...
//import soc.game._
//import soc.game.resources.SOCPossibleHands
...
//    }
//
//  def selectBestMove[M <: SOCPossibleMove](moves: List[M]): M = moves(rand.nextInt(moves.length))
//
...
//    val state = GameState(getGame, getGame.getCurrentPlayerNumber, possibleHands)
//    val moves = GameState.getPossibleDiscards(getOurPlayerData, numToDiscard)
//    val bestDiscard = selectBestMove(moves)
